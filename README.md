...
3. **绑定域名**：在域名提供商处添加指向 GitHub Pages 的 CNAME 记录（`用户名.github.io`），并在仓库根目录新增 `CNAME` 文件写入你的自定义域名。本仓库已经在根目录放置 `CNAME` 文件并写入 `www.yanlingtech.top`，只需将仓库 push 到 GitHub 后在 Pages 设置中保存即可。
